<?php $this->load->view('spp/spp/ubah/form3') ?>
...
<table id="rincian" class="table table-condensed table-bordered" style="width:100%;">
	<thead>
		<tr>
			<th>Rekening</th>
			<th>Uraian</th>
...
function _ready_rician() {
	var v1 = data.spp.KODEURUSAN + '-' + data.spp.KODESUBURUSAN + '-' + data.spp.KODEORGANISASI + '-' + data.spp.KODEDESA;
	var v2 = $('#spp_inp_keg').val();
	var p1 = data.spp.NO_SPP;
	var p2 = $form[0].TGL_SPP.value;
	var param = v1 + '/' + v2 + '?nomor=' + encodeURIComponent(p1) + '&tanggal=' + p2;

	if (v2 !== null && p2 !== '') { // buat tabel
		_get_rincian(param);
	} else { // reset tabel
		RincianData = $('#rincian').DataTable({
			...
			bLengthChange: false,
			data: [],
			language: $.extend({}, id_dttable, {zeroRecords: "Isi <b>Kegiatan</b>, <b>Nomor SPP</b> dan <b>Tanggal SPP</b> untuk menampilkan rincian."})
		});
	}
}
function _get_rincian(param) {
	is_wroking_keg = true;
	if (typeof data.rincian[param] === 'undefined') {
		$.get(base_url + 'svc/spp_angg/' + param, function(dat) {
			...
			data.rincian[param] = dat;
			_table_rincian(dat);
		});
	} else {
		_table_rincian(data.rincian[param]);
	}
}
